        ...
        Object.entries(data).forEach(([itemname, item]) => {
            _getAllItemVariants(item, itemname).forEach(({ prefixKey, displayLabel }, varIdx) => {
                ...
                if (attrs.length > 0) {
                    ...
                    tbodyHtml += `<tr style="background:rgba(100,114,168,0.08);"><td style="text-align:center;padding:2px 8px;color:#b0bcff;font-size:0.75em;white-space:nowrap;border-right:1px solid #2a3158;">속성</td><td colspan="2" style="padding:3px 10px;">${attrHtml}</td></tr>`;
                }
                ...
                if (desc) {
                    ...
                    tbodyHtml += `<tr><td style="text-align:center;padding:2px 8px;color:#c8b87a;font-size:0.75em;white-space:nowrap;border-right:1px solid #2a3158;">설명</td><td colspan="2" style="padding:4px 10px;color:#c8b87a;font-size:0.8em;text-align:left;vertical-align:top;">${fmtDesc}</td></tr>`;
                }
            });
        });

        tableWrap.innerHTML = `<table style="border-collapse:collapse;width:max-content;min-width:340px;"><thead><tr><th style="padding:4px 8px;text-align:center;font-size:0.8em;width:90px;border-right:1px solid #2a3158;">구분</th><th style="padding:4px 8px;text-align:center;font-size:0.8em;">스탯</th><th style="padding:4px 8px;text-align:center;font-size:0.8em;width:80px;border-left:1px solid #2a3158;">수치</th></tr></thead><tbody>${tbodyHtml}</tbody></table>`;
