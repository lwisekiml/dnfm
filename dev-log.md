...
### 엠블렘 비교 두 번째 캐릭터 열 순서 수정

**수정된 파일:** `js/ui-compare.js`
...
- `_makeDescEditable()` 함수 수정 — 칭호/외형칭호, 오라, 크리쳐 설명 클릭 시 자동입력 상태 확인 로직 추가
  - **칭호/외형칭호**: `TITLE_ITEM_INFO[이름]`에 `info` 또는 `desc` 필드가 있으면 자동입력으로 판단
  - **오라**: `AURA_ITEM_INFO[이름]`에 `info` 또는 `desc` 필드가 있으면 자동입력으로 판단
  - **크리쳐**: 버튼의 `data-creature-setauto` 속성이 `'true'`이면 자동입력으로 판단
  - 자동입력 상태일 경우 alert 표시 후 수정 불가
    - 메시지: *"자동입력된 설명은 수정할 수 없습니다. 팝업에서 아이템 이름을 직접 입력하면 수정 가능합니다."*
  - 자동입력이 아닌 경우(사용자가 직접 입력한 이름) 기존처럼 textarea로 편집 가능

**효과**

- 데이터 무결성 보호: 자동입력된 표준 설명은 검색 화면에서 실수로 수정되는 것 방지
- 사용자 편의성: 직접 입력한 커스텀 아이템은 여전히 자유롭게 수정 가능
- 일관된 UX: 팝업에서만 아이템 이름을 변경/입력하도록 유도
...
### 칭호/오라 팝업 직접 입력 기능 개선

**수정된 파일:** `js/ui-character.js`, `index.html`

---

### 변경 내용

#### 1. 직접 입력 시 스탯과 설명 저장 문제 수정

**문제:**
- 칭호/오라 팝업에서 직접 입력한 경우, 이름만 저장되고 수치(스탯)와 설명이 저장되지 않음

**원인:**
- `titlePopupSave()` / `auraPopupSave()` 함수에서 desc 반영 시 사용자가 입력한 `stats.desc` 값을 사용하지 않음

**해결:**
- 설명 반영 로직 수정:
  1. 자동입력 데이터 있음 → `infoEntry.info` 사용
  2. 직접 입력 + 스탯 기반 자동생성 + 사용자 설명 모두 표시
  ```
  기본정보
  힘, 지능 +50
  ---
...
#### 2. 팝업 재열람 시 입력값 복원 문제 수정

**문제:**
- 직접 입력한 칭호/오라의 팝업을 다시 열면 이전에 입력했던 스탯과 설명이 표시되지 않음

**원인:**
- `openTitlePopup()` / `openAuraPopup()` 함수에서:
  1. 저장된 값 복원
  2. `setTimeout`으로 `titleNameApplyStats()` / `auraNameApplyStats()` 호출
  3. 자동입력 데이터가 없으면 모든 input을 비워버림
  4. 결과: 복원한 값이 다시 지워짐

**해결:**
- 팝업 열기 시 자동입력 여부를 먼저 확인:
  - **자동입력 데이터 있음**: `titleNameApplyStats()` / `auraNameApplyStats()` 호출 (기존 로직)
  - **직접 입력**: 저장된 값 복원만 수행, 자동입력 함수 호출하지 않음

#### 3. 이름 타이핑 시 스탯 초기화 문제 수정

**문제:**
- 이름을 직접 입력하고 수치를 입력한 후, 이름 칸을 클릭하거나 타이핑하면 입력했던 수치가 모두 사라짐

**원인:**
- `titleNameDropdownShow()` / `auraNameDropdownShow()` 함수가 타이핑할 때마다 `titleNameApplyStats()` / `auraNameApplyStats()` 호출
- 데이터에 없는 이름이면 모든 스탯 input을 비움

**해결:**
- 타이핑 시 스탯 갱신 코드 제거
- **드롭다운에서 선택했을 때만** 자동입력 적용

#### 4. 스탯 그룹별 정렬 및 출력 기능 추가

**`_STAT_GROUPS` 상수 추가:**
- 13개 스탯 그룹 정의 (힘/지능/체력/정신력, 적중/회피, 공속/캐속/이속 등)

**`_entriesToLines()` 함수 재작성:**
- 같은 그룹 내에서 **같은 수치**를 가진 스탯들을 쉼표로 묶어 한 줄에 출력
- 다른 수치는 별도 줄로 출력
- 그룹이 다르면 수치가 같아도 별도 줄로 출력

**예시:**
```
입력: 힘:1, 지능:1, 체력:2, 적중:2, 회피:2, 공격속도:2, 캐스팅속도:2, 이동속도:1
...
**`index.html`**
- 칭호/오라 팝업 이름 입력란 옆에 "🔄 수치/설명 초기화" 버튼 추가

**`js/ui-character.js`**
- `resetTitlePopupStats()` / `resetAuraPopupStats()` 함수 추가
- 자동입력 상태에서는 초기화 불가 (alert 표시)
- 직접 입력한 경우에만 모든 스탯과 설명 초기화

#### 6. 이름 수정 시 실시간 자동입력 상태 UI 업데이트

**문제:**
- 자동입력 상태에서 이름을 수정하면 실제로는 자동입력이 아닌데 화면에만 자동입력 badge가 표시되고 입력이 잠김

**해결:**
- `titleNameDropdownShow()` / `auraNameDropdownShow()` 함수에 실시간 체크 로직 추가
- 이름 타이핑할 때마다 자동입력 데이터 조회
- 데이터가 **없으면** → badge 숨김 + 잠금 해제 (즉시 반영)
- 데이터가 **있으면** → 드롭다운 선택 시에만 자동입력 적용

**시나리오:**
1. "성장의 칭호" 선택 (자동입력 badge 표시, 입력 잠김)
2. 이름을 "테스트칭호"로 수정
3. **즉시** badge 숨김 + 입력 잠금 해제
4. 스탯과 설명 수정 가능
...
### 방어구 세트 이름 수정 및 캐릭터 관리 - 비교에서 방어구 스탯, 세트 효과 비교 추가

**수정된 파일:** `js/ui-compare.js`, `data/armor_set_effects.js`

### 변경 내용

1. 방어구 스탯 비교 표 개선(`ui-compare.js` - `buildArmorStatCompare()`)

- **스탯 데이터 구조 개선**
  - 기존: `{ [스탯명]: amount }` 형태의 단순 맵
  - 변경: `{ [스탯명]: { amount, unit } }` 형태로 단위(unit) 정보 포함

- **표 레이아웃 개선**
  - 컬럼 수 변경: 5개 → 7개 (좌우 대칭 구조)
  - 좌측: `[구분] [스탯] [캐릭터1 수치]`
  - 중앙: `[차이]`
  - 우측: `[캐릭터2 수치] [스탯] [구분]`
  - 시각적 구분을 위한 세로 구분선(`border-right`) 추가

- **수치 표시 개선**
  - 단위 정보 표시 추가 (예: `50%` → `50` + `%`)
  - 0값 처리: 0인 경우 빈 문자열로 표시하여 가독성 향상
  - 차이값 계산 로직 개선:
```javascript
    // 기존: 단순 차이값만 표시
    // 변경: 상황별 차이 표시
    - 양쪽 모두 값이 있을 때: 차이 계산 (↑ +5%, ↓ -3%, 동일)
    - 한쪽만 값이 있을 때: 해당 값으로 증감 표시
    - 양쪽 모두 0: 차이 미표시
```

2. 방어구 세트 효과 비교 표 신규 추가(`ui-compare.js` - `buildArmorSetEffectCompare()`)
...
function getArmorSetEffects(section) {
  // 착용 아이템 수집
  // → 세트별 그룹화 (setName: { prefix: count })
  // → 가장 많은 개수의 세트 선택 (접두어 우선)
  // → 3세트/5세트 효과 데이터 반환
}
```
...
#### 시각적 구분
- 3세트 효과: 하늘색 헤더 (`rgba(127,212,255,0.15)`)
- 5세트 효과: 금색 헤더 (`rgba(255,215,0,0.15)`)
- 차이가 있는 행: 배경 하이라이트
- 증가/감소: 색상 구분 (초록/빨강)

### 3. displayComparison() 함수 수정
**변경 내용**
```javascript
// 방어구 세트 효과 비교 표 추가
...
### 데이터 의존성
- `ARMOR_ITEM_STATS`: 방어구 스탯 정보 (기존)
- `ARMOR_ITEM_INFO`: 방어구 세트 정보 (신규 필요)
- `ARMOR_SET_EFFECTS`: 세트 효과 정보 (신규 필요)

### 데이터 구조
```javascript
// ARMOR_SET_EFFECTS 예상 구조
{
  "세트명": {
    "접두어": {
      "3": { stats: [{ stats: ["스탯명"], amount: 값, unit: "단위" }] },
      "5": { stats: [...] }
    }
  }
}
...
- 단위 정보 포함
- 7컬럼 좌우 대칭 레이아웃
- 가독성 향상 (0값 숨김, 시각적 구분선)

## 🐛 알려진 이슈
- 없음

## 📝 다음 작업
- `ARMOR_ITEM_INFO`, `ARMOR_SET_EFFECTS` 데이터 파일 생성 필요
- 세트 효과 툴팁 추가 고려
- 모바일 반응형 레이아웃 테스트 필요

## 💡 참고사항
- 기존 `buildArmorStatCompare()` 함수의 로직은 유지하되 표시 형식만 개선
- 새로운 `buildArmorSetEffectCompare()` 함수는 독립적으로 동작
- 두 표 모두 `compare-section-wrapper` 클래스 사용하여 일관된 스타일 적용
...
### 비교에서 레거시 표시 수정 및 세트 오류 수정 + 방어구 세트 업데이트
...
**`buildArmorSetEffectCompare`, `buildAccSetEffectCompare` 함수 수정**
- 기존: "레거시: 레거시:" 로 표시
- 변경: "레거시:" 로 표시
...
**`renderCharacterEquipmentDetail` 함수 수정**
- 기존: 카테고리(방어구/악세/특장)당 테이블 1개, 헤더에 슬롯명(`상의`, `하의` 등) 표시, 데이터 셀 숫자 아래에 아이템명 표시
- 변경: 세트마다 별도 테이블 생성, 헤더에 실제 아이템명 표시, 데이터 셀에서 아이템명 제거
- 익시드/일반 아이템명이 모두 있는 슬롯(상의/팔찌/귀걸이)은 헤더에 항상 2줄 표시
  - 익시드 아이템명: 파란색(`#aad4ff`)
  - 일반 아이템명: 흰색(`#fff`)
...
### 무기 침식 여부에 따른 이미지 분기 처리

**수정된 파일:** `character/base-character.js`, `character/base-core.js`
...
- `updateWeaponImage` 함수 수정
  - 익시드 select(`무기_exceed`) 값을 읽어 침식 여부 확인
  - 침식일 경우 이미지 파일명 앞에 `침식_` 접두어 추가
    - 예: `암흑의 별` + 침식 → `침식_암흑의 별.png`
  - 침식이 아닐 경우 기존과 동일하게 아이템명 그대로 사용

**`character/base-core.js`**

- `updateStyle` 함수의 exceed 처리 부분 수정
  - 무기 슬롯에서 익시드 값 변경 시 `updateWeaponImage` 호출 추가
  - 침식 선택/해제 시 이미지가 즉시 갱신되도록 처리

---

### 변경 이유

- 침식 무기는 일반 무기와 외형이 다르므로 별도 이미지로 구분
- 이미지 파일명 규칙을 기존 방어구 접두어 이미지 방식(`접두어_아이템명.png`)과 동일하게 통일
